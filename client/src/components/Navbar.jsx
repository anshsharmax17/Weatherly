  ...
  return (
    <header className="sticky top-4 z-30 mx-auto w-[min(1120px,calc(100%-24px))]">
      <nav className="glass flex flex-wrap items-center justify-between gap-3 rounded-3xl px-4 py-3">
        <div className="flex items-center gap-3 text-xl font-extrabold text-slate-950 dark:text-white">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {!isAuthenticated ? <Link className="btn-primary" to="/login">Sign in</Link> : <button onClick={handleLogout} className="flex items-center gap-2 rounded-2xl bg-rose-500 px-4 py-2 font-semibold text-white"><LogOut size={16} /> Logout</button>}
          <ThemeToggle />
        </div>
      </nav>
    </header>
